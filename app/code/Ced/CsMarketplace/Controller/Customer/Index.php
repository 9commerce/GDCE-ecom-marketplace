...
/**
 * Class Index
 * @package Ced\CsMarketplace\Controller\Customer
 */
class Index extends \Ced\CsMarketplace\Controller\Vorders
{
    ...
    {
        if(!$this->csmarketplaceHelper->canHideCustomerDetails()){
            $this->messageManager->addError(__('Not Allowed to see customer details. Contact Store Owner for more details'));
            $resultRedirect = $this->resultRedirectFactory->create();
            $resultRedirect->setPath('csmarketplace');
            return $resultRedirect;
        }
        $resultPage = $this->resultPageFactory->create();
        $resultPage->getConfig()->getTitle()->set(__('Customer List'));
        return $resultPage;
    }
}
