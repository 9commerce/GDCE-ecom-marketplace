...
class CustomOptions extends AbstractModifier
{
     ...
     * @param Http $request
     */
    public function __construct(LocatorInterface $locator, ArrayManager $arrayManager, Http $request)
    {
    ...
    private function addPriceTypeWarning(array $meta)
    {
        ...
        if ($gridPath) {
            ...
            $content = __('Custom options with price type "percent" is not available for configurable product.');
            $typeId = $this->request->getParam('type');
            if ($typeId==ConfigurableProductType::TYPE_CODE) {
