...
class Eav extends \Magento\Catalog\Ui\DataProvider\Product\Form\Modifier\Eav
{
    ...
    private function loadAttributesForGroups(array $groups)
    {
        $attributes = [];
        $groupIds = [];
        $hideGroups = ['schedule-design-update', 'design', 'gift-options', 'subscriptions-by-stripe'];
        foreach ($groups as $group) {
            if (in_array($group->getAttributeGroupCode(), $hideGroups)) {
                continue;
            }
        ...
        $hideAttributes = ['ced_rpoint', 'dispatch_time'];

        foreach ($groupAttributes as $attribute) {
            if (in_array($attribute->getAttributeCode(), $hideAttributes)) {
                continue;
            }
