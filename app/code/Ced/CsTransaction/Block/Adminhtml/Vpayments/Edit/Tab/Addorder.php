...
class Addorder extends \Ced\CsMarketplace\Block\Adminhtml\Vpayments\Edit\Tab\Addorder
{
    ...
    public function getSearchFormHtml()
    {
        if ($this->csorderHelper->isActive()) {
            ...
            $collection = $this->_vtItemCollectionFactory->create()->addFieldToFilter('main_table.vendor_id', ['eq' => $vendorId]);
            ...
            $collection->getSelect()
                ->joinLeft(
                    'ced_csmarketplace_vendor_payments_requested',
                    'ced_csmarketplace_vendor_payments_requested.vorder_item_id = main_table.id',
                    ['*']
                );
            if(isset($params['order_ids'])){
                $collection = $collection->addFieldToFilter('ced_csmarketplace_vendor_payments_requested.vorder_item_id', ['in' => $params['order_ids']]);
            }
